...
void Trigger_Efficiency_calc_HLT_EleLeg_HT( int maxNentries=-1, int Njobs=1, int jobN=1 ) {
  ...
  for (Long64_t ievt=0; ievt<chain->GetEntries();ievt++) {    //Long64_t
      ...
      //Grab Specific Lepton information from trees
	    ...
	    //if( pass_HLT_PFHT180 != 1 && pass_HLT_PFHT250 != 1 && pass_HLT_PFHT350 != 1 && pass_HLT_PFHT370 != 1 && pass_HLT_PFHT430 != 1 && pass_HLT_PFHT510 != 1 && pass_HLT_PFHT590 != 1 && pass_HLT_PFHT680 != 1 && pass_HLT_PFHT780 != 1 && pass_HLT_PFHT890 != 1 && pass_HLT_PFHT1050 != 1 )
		if( pass_HLT_PFHT250 != 1 && pass_HLT_PFHT430 != 1 && pass_HLT_PFHT510 != 1 && pass_HLT_PFHT590 != 1 && pass_HLT_PFHT680 != 1 && pass_HLT_PFHT780 != 1 && pass_HLT_PFHT890 != 1 && pass_HLT_PFHT1050 != 1 )
		    continue;
        ...
        for(unsigned int i=0; i<ele_pt.size(); i++) {
            ...
            if(ele_pt[i] > 10 && fabs(ele_eta[i]) < 2.4 && !inCrack && ele_is_ID_Tight[i] == 1) {
                nele_loose++;
                if(ele_pt[i] > 10 && fabs(ele_eta[i]) < 2.1) {
                    nele_tight++;
        ...
        if(nele_tight == 1){
            if( nele_loose == 1 ){
                if(njets >= 1 && HT > 200){
					...
					int ele_ht_label_match = 0;
					int ele_ht_dR_match_index = 0;
					int ele_ht_sig_hlt_match = 0;
					...
					for(unsigned int l=0; l<eta_trigger_object.size(); l++){
						...
						if (check_filter_label( filter_trigger_object[l] , "hltSinglePFJet40" ) == 1 || check_filter_label( filter_trigger_object[l] , "hltSinglePFJet60" ) == 1 || check_filter_label( filter_trigger_object[l] , "hltSinglePFJet80" ) == 1 || check_filter_label( filter_trigger_object[l] , "hltSinglePFJet140" ) == 1 || check_filter_label( filter_trigger_object[l] , "hltSinglePFJet200" ) == 1 || check_filter_label( filter_trigger_object[l] , "hltSinglePFJet260" ) == 1 || check_filter_label( filter_trigger_object[l] , "hltSinglePFJet320" ) == 1 || check_filter_label( filter_trigger_object[l] , "hltSinglePFJet400" ) == 1 || check_filter_label( filter_trigger_object[l] , "hltSinglePFJet450" ) == 1 || check_filter_label( filter_trigger_object[l] , "hltSinglePFJet500" ) == 1 || check_filter_label( filter_trigger_object[l] , "hltSinglePFJet550" ) == 1)
							ele_ht_label_match = 1;
						//ele_ht_label_match = check_filter_label( filter_trigger_object[l] , "hltPFHT150Jet30" );
						if (ele_ht_label_match == 1){
							dR = dR_calc(electron_eta, electron_phi, eta_trigger_object[l], phi_trigger_object[l]);
							if( dR < 0.3 ){
								if( dR < dR_min5){
									ele_ht_dR_match_index = l;
									ele_ht_sig_hlt_match = 1;
									dR_min5 = dR;
								}
							}
						}
					...
					int no_overlap = 0;
					//ele_ht_sig_hlt_match = 0;

					if(pass_HLT_Ele27_WPTight_Gsf != -1 && prescale_HLT_Ele27_WPTight_Gsf != 0 && ele_ht_sig_hlt_match == 0){
						N_e_den11++;
						no_overlap = 1;

						int pass_ele27 = ele_sig11_hlt_match * no_overlap;
                    ...
                    if(pass_HLT_Ele32_WPTight_Gsf != -1 && prescale_HLT_Ele32_WPTight_Gsf != 0 && ele_ht_sig_hlt_match == 0){
                        N_e_den33++;
                        no_overlap = 1;

                        int pass_ele32 = ele_sig33_hlt_match * no_overlap;
                    ...
                    if(pass_HLT_Ele32_WPTight_Gsf_L1DoubleEG != -1 && prescale_HLT_Ele32_WPTight_Gsf_L1DoubleEG != 0 && ele_ht_sig_hlt_match == 0){
                        N_e_den44++;
                        no_overlap = 1;

                        int pass_ele32_l1eg = ele_sig44_hlt_match * no_overlap;
					...
					if(pass_HLT_Ele35_WPTight_Gsf != -1 && prescale_HLT_Ele35_WPTight_Gsf != 0 && ele_ht_sig_hlt_match == 0){
						N_e_den22++;
						no_overlap = 1;

						int pass_ele35 = ele_sig22_hlt_match * no_overlap;
					...
					if(pass_HLT_Ele30_eta2p1_WPTight_Gsf_CentralPFJet35_EleCleaned != -1 && prescale_HLT_Ele30_eta2p1_WPTight_Gsf_CentralPFJet35_EleCleaned != 0 && jet_sig2_hlt_match == 1 && ele_ht_sig_hlt_match == 0 ){
						...
						*/
						...
						//int pass_ele30_jet35 = pass_HLT_Ele30_eta2p1_WPTight_Gsf_CentralPFJet35_EleCleaned * ele_sig1_hlt_match * no_overlap;
						int pass_ele30_jet35 = ele_sig1_hlt_match * no_overlap;
					...
					if(pass_HLT_Ele28_eta2p1_WPTight_Gsf_HT150 != -1 && prescale_HLT_Ele28_eta2p1_WPTight_Gsf_HT150 != 0 && jet_sig2_hlt_match == 1 && ele_ht_sig_hlt_match == 0 ){

						N_e_den2++;

						//int pass_ele28_ht150 = pass_HLT_Ele28_eta2p1_WPTight_Gsf_HT150 * ele_sig2_hlt_match;
						int pass_ele28_ht150 = ele_sig2_hlt_match;
