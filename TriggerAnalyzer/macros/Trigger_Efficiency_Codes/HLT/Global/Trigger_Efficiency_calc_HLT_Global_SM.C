...
void Trigger_Efficiency_calc_HLT_Global_SM( int maxNentries=-1, int Njobs=1, int jobN=1 ) {
  ...
  for (Long64_t ievt=0; ievt<chain->GetEntries();ievt++) {    //Long64_t
	    ...
	    if( pass_HLT_IsoMu30 != 1 && pass_HLT_IsoMu27 != 1 && pass_HLT_IsoMu24_eta2p1 != 1 && pass_HLT_IsoMu24 != 1 )
		  continue;
        ...
        // Single Electron event selection (also required exactly 1 offline muon)
        if(nele_tight == 1){
            if( nele_loose == 1 && nmu_tight == 1 && nmu_loose == 1 ){
                if(njets >= 1){
                    N_e++;
